...
(* FILL IN HERE
The [rewrite] tactice is used to apply a known equality to either the goal or a
hypothesis in the context, replacing all occurrences of one side by the other.
The [apply] tactic uses a known implication (a hypothesis from the context, a
previously proved lemma, or a constructor) to modify the proof stata either
backward (if the goal matches the conclusion of the implication, in which case a
subgoal is generated for each premise of the implications) or forward (if some
hypothesis matches the premise of the implication, in which case this hypothesis
is replaced by the conclusion of the implication). If the fact is itself an
equality (i.e., an implication with no premises), then either tactic can be
used.
    [] *)

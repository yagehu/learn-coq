...
Proof.
  ...
  rewrite -> add_comm.
  rewrite <- add_assoc.
  assert (H: p + n = n + p). { rewrite -> add_comm. reflexivity. }
  rewrite -> H.
  reflexivity.
Qed.
...
(*
  The representation of a binary number is not unique.
*)
...
Lemma nat_to_bin_double:
  forall n: nat, nat_to_bin (double n) = double_bin (nat_to_bin n).
Proof.
  induction n as [| n' IHn' ].
  - reflexivity.
  - simpl.
...
Lemma incr_double:
  forall b: bin, incr (double_bin b) = B1 b.
Proof.
  destruct b.
  - reflexivity.
  - reflexivity.
